...
def encrypt(text, key):
    '''Takes in a string of letters (non alpha characaters and white spaces are ignored) along with a numeric shift and returns a string'''
...
def decrypt(text, key):
    ...
    if not isinstance(key, int):
        raise ValueError("The key argument has to be an integer")
    if not isinstance(text, str):
        raise ValueError("Please enter the next argument as a string")
...
if __name__ == "__main__":
    ...
    print (decrypt("Lxx max ebzam!", -7)) 
    print (break_code("You done it now"))